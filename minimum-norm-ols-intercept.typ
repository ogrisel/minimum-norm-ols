...
$ & &min_(w, w_0) 1/2 (w^T w + gamma w_0^2) \
  &"s.t." &X w + w_0 1_n = y $  <problem>
...
- $X_c$ is the centered version of $X$ such that $X_c = X - 1_n overline(X)^T$;
...
$ X = X_c +  1_n overline(X)^T $
$ y = y_c + overline(y) 1_n $

We can rewrite the generic formulation of the problem in @problem as:

$ & &min_(w, w_0) 1/2 (w^T w + gamma w_0^2) \
  &"s.t." &X_c w +  1_n overline(X)^T w
   + w_0 1_n = y_c + overline(y) 1_n $

Let's introduce Langrange multipliers $lambda$ to define our unconstrained
objective function:

$ L(w, w_0, lambda) = &1/2 w^T w + gamma/2 w_0^2 \
  &+ lambda^T X_c w
   + (lambda^T 1_n) (overline(X)^T w)
   + w_0 lambda^T 1_n \
  &- lambda^T y_c - overline(y) lambda^T 1_n  $

The minimizer of this objective function is a critical point:

- $nabla L_w(w, w_0, lambda) = 0_p$ yields:

$ w + X_c^T lambda + (lambda^T 1_n) overline(X) = 0_p $ <grad_w>

- $nabla L_w_0(w, w_0, lambda) = 0$ yields:

$ gamma w_0 + lambda^T 1_n = 0 $ <grad_w_0>

- $nabla L_lambda(w, w_0, lambda) = 0_n$ yields:

$ X_c w + (overline(X)^T w) 1_n
   + w_0 1_n = y_c + overline(y) 1_n $ <grad_lambda>

Right-multiplying @grad_lambda by $1_n^T$ yields:

$ 1_n^T X_c w + (1_n^T 1_n) (overline(X)^T w)
   + w_0 (1_n^T 1_n) = 1_n^T y_c + overline(y)1_n^T 1_n $

Since $1_n^T 1_n = n$, $1_n^T X_c = 0_p$ and
$1_n^T y_c = 0$ we recover the usual:

$ w_0 = overline(y) - overline(X)^T w $ <w_0>
...
$ w  = - X_c^T lambda $ <w_from_lambda>

and therefore:

$ w_0 = overline(y) + overline(X)^T X_c^T lambda $ <w_0_from_lambda>

Let's subtitute in $w_0$ and $w$ in @grad_lambda:

$ - X_c X_c^T lambda
  - (overline(X)^T X_c^T lambda) 1_n
  + (overline(y) + overline(X)^T X_c^T lambda) 1_n
  = y_c + overline(y) 1_n $

Hence, after simplification, and assuming $X_c X_c^T$ is invertible:

$ lambda = - (X_c X_c^T)^(-1) y_c $ <lambda>

and therefore the solution is:

$ &hat(w) &= - X_c^T (X_c X_c^T)^(-1) y_c \
  &hat(w)_0 &= overline(y) - overline(X)^T hat(w) $
